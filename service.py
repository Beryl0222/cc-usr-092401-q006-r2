        ...
        def do_GET(self):
            ...
            try:
                ...
                if path == "/batches":
                    ...
                    state = qs.get("状态", [None])[0]
                    self._send(200, {"记录": self.svc.list_conflicts(actor, state)})
        ...
        def do_POST(self):
            ...
            try:
                ...
                if path == "/farmers":
                    ...
                    self._send(200, svc.resolve_ticket_conflict(
                        actor, data["冲突编号"], data["复核结论"],
                        data.get("复核意见", "")))
                else:
                    self._send(404, {"error": "not_found", "message": "未知路由"})
